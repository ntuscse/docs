...
But, what is serverlss computing? Does there mean there are no servers involved? Well, serverless computing still has servers to host application data for deployment,
it abstracts away the need to manage servers for application developers. How it achieves this is by providing FaaS(Function as a Service) platform whereby 
...
Now that we've takena look at some functionalities offered by AWS Lambda, how exactly does it work, and how can you, as an app or API developer leverage the 
functionlities of AWS Lambda?
...
Since our function is a offline task, a brute force search of all the EC2 instances will suffice, however, as an app or API developer you may want to consider
the runtime complexity of your Lambda functions for online execution tasks. We'll use Python to code our Lambda function, but you can write th logic in any of the 
supported languages.
...
is not necessary to use thee arguments in the fucntion body, nonetheless, every Lambda function must have these as function arguments. A Lambda function may 
have more arguments too, dpeending upon the nature of the event that triggers it. The logic of the function is pretty simple. We perform a brute force search of all
...
8. Now that your Lambda function is created, we need to add an EVent rule which dictates when the Lambda function will be triggered. Click on your Lamda function
and click on Test to test your function. If everything goes fine, you should be status Successful. Additionally if you want, you could actually try running EC2 
instances and Testing the Lambda function top see if the function actually stops the instances or not. It is a best practice to thoroughly test your Lambda
...
that monitors events at periodic times in the AWS console, which is relevant for us since ourt Lambda function is periodic too.
...
go ahead and chooe Schedule expression.
...
13. At this point, your AS Lambda function is complete, and ready to be deployed. Head back to the Lambda funtion page, and clikc on Deploy. Note: you may or may not
...
* reptitive scheduled tasks that must occur with a given schedule - these can be easily auotmated in a similar manner using AWS Lambda as we have demonstarted in the
example above.
...
aggregate data by a defined logic based on an input data from Amazon S3, or other apps/databases.
